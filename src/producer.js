...
async function sendBatch(kafkaMessages) {
    try {
        ...
        // O tratamento já foi feito em sendBatchWithRetry
    }
}
...
async function processBatchesLoop() {
    while (true) {
        try {
            // Primeiro tenta processar a fila de retry, se houver mensagens
            if (retryQueue.length > 0) {
                await processRetryQueue();
            }
            ...
            if (messages.length === 0) {
                ...
                // Se não há novas mensagens, tenta reprocessar a dead letter queue
            ...
            // Pequena pausa para não sobrecarregar CPU/memória
...
async function shutdown() {
    ...
    try {
        // Se houver mensagens na fila de retry, salva em dead letter antes de encerrar
        if (retryQueue.length > 0) {
